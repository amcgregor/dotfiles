...
def saturate_color(color, context, amount=None):
    ...
    if isinstance(amount, Value):
        if amount.unit == '%':
            if not amount.value:
                return color
            saturation *= 1.0 + amount.value / 100.0
        else:
            raise EvalException(self.lineno, 'invalid unit %s for color '
                                'calculations.' % amount.unit)
    elif isinstance(amount, Number):
        saturation += (amount.value / 100.0)
    if saturation > 1:
        saturation = 1.0
    return Color(hls_to_rgb(hue, lightness, saturation))


def desaturate_color(color, context, amount=None):
    if amount is None:
        amount = Value(10.0, '%')
    hue, lightness, saturation = rgb_to_hls(*color.value)
    if isinstance(amount, Value):
...
class String(Literal):
    ...
    def neg(self, context):
        return String("-" + self.value, lineno=self.lineno)
...
class Backstring(Literal):
    ...
    def to_string(self, context):
        if isinstance(self.nodes, unicode):
            return self.nodes
        return unicode(self.nodes, 'utf8')
    ...
    # convert some files
        ...
        for fn in sys.argv[1:]:
            ...
            try:
                ...
                dst = file(target, 'wb')
                try:
                    dst.write(converted.encode('utf8'))
                finally:
                    dst.close()
            finally:
                src.close()
